<template>
  <v-card style="display: flex; flex-direction: column; height: 100%">
    ...
    <v-card-subtitle>{{ moment(chronicle.createdAt).format(`${$ml.get("dateFormat")}`) }}</v-card-subtitle>
    <v-card-text style="flex: 1">{{ chronicle.shortDescription }}</v-card-text>
    <v-card-actions>
      <v-chip v-if="player">
        <v-avatar left>
          <v-img :src="chronicle.storyTellerPicture"></v-img>
        </v-avatar>
        {{chronicle.storyTellerName}}
      </v-chip>
      <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    openChronicle(id) {
      this.$router.push({
        path: `${this.$route.path}/chronicle/${id}/${
          this.$route.path === "/story-teller" ? "dashboard" : "characters"
        }`
      });
    }
  }
